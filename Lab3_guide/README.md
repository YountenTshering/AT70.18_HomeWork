...
ORM using Hibernate, JPA, and Spring Boot.  These concepts include:
...
As a case study, we will be making an employee domain model with the following attributes:
1. id as primary key
2. Name
3. age
4. Addresses with OneToMany relationship
5. Benefits with ManyToMany relationship 
6. Leaves with OneToMany relationship (leave to annual leave or sick leave)
7. User with OneToOne relationship (user to employee)
...
1. Next, set up the application.properties for your datasource

![alt](./ORM/image/3.PNG)

Cache

1. We also have to inform our Hibernate to use 2nd level cache.  In addition, we will be using ENABLE_SELECTIVE in which cache only applies to those with @Cache and not all entities.  Last, we also want to make sure we inform hibernate the factory that will be  performing the 2nd-level cache.  Note that I did not turn on query_cache, but if you want to try, you can certainly turn in on and use along with @QueryHints in the repository
...
need to be cached and there are no caching regions defined for that.  For example, timeToLiveSeconds was set to 10, meaning that things will be kept in cache for 10 seconds.  Normally you would set higher numbers, but for demonstration, set it to a much lower number
