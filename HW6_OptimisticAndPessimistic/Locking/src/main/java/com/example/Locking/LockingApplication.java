...
public class LockingApplication {
	...
	private static String get(int id) throws IOException, InterruptedException {
		HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080/products/" + id))
	...
	private static void insert(String name, int price) throws IOException, InterruptedException {
		...
		HttpRequest request = HttpRequest.newBuilder().POST(HttpRequest.BodyPublishers.ofString(json)) // .POST accepts
																										// a
																										// BodyPublisher
				.uri(URI.create("http://localhost:8080/products")).setHeader("User-Agent", "Java 11 HttpClient Bot")
	...
	private static void update(String jsonString) throws IOException, InterruptedException {
		HttpRequest request = HttpRequest.newBuilder().PUT(HttpRequest.BodyPublishers.ofString(jsonString))
				.uri(URI.create("http://localhost:8080/products/")).setHeader("User-Agent", "Java 11 HttpClient Bot")
				.header("Content-Type", "application/json").build();
	...
	private static String makeJSON(String name, int price) {
		String json = new StringBuilder().append("{").append("\"name\":\"" + name + "\",")
				.append("\"price\":\"" + price + "\"").append("}").toString();
		return json;
	}
	...
	private static String convertObjToJSON(Product product) throws JsonProcessingException {
