...
public class ThreeWayRecognitionStrategy implements RecognitionStrategy {
    ...
    public void calculateRevenueRecognitions(Contract contract) {
        ...
        BigDecimal allocations[] = allocate(revenue.getNumber().numberValue(BigDecimal.class)
                .setScale(revenue.getCurrency().getDefaultFractionDigits(), RoundingMode.HALF_EVEN), 3);
        contract.addRevenueRecognition(amountFactory.setCurrency(Contract.CURRENCY).setNumber(allocations[0]).create(),
                dateSigned);
        contract.addRevenueRecognition(amountFactory.setCurrency(Contract.CURRENCY).setNumber(allocations[1]).create(),
                dateSigned.plusDays(firstRecognitionOffset));
        contract.addRevenueRecognition(amountFactory.setCurrency(Contract.CURRENCY).setNumber(allocations[2]).create(),
